...
var serverData = {
    'jou':{
        ...
        node:["mysterygamenode-journalsofunknown.azurewebsites.net","mysterygamenode02-journalsofunknown.azurewebsites.net"],
        dollarConversion:0.1,
        gifs:["401.gif","402.gif","403.gif","404.gif","405.gif","406.gif","407.gif","408.gif","409.gif","410.gif","411.gif","412.gif"]
    },
    'sherlock':{
        name:"sherlock",
        counters:{fb:"mysterygame-counters-sherlock.azurewebsites.net"},
...
function updateCaching(originalResponse) {
        ...
        for(var i = 0; i < 10; i++) {
            groups.push({money: 0, group: i, payers: []});
        }
        ...
        for(var item in response) {
            ...
            var userabgroup = response[item].userId.substr(response[item].userId.length - 1);
            groups[userabgroup].money += response[item].price;
            if(groups[userabgroup].payers.indexOf(response[item].userId) == -1) groups[userabgroup].payers.push(response[item].userId);
        ...
        for(var i = 0; i < groups.length; i++) {
            document.getElementById('bygroup').innerHTML += '<b>$' + toDollars(groups[i].money) + '</b> Group ' + groups[i].group + '<span class="details">' + ' <b>Payers:</b> ' + groups[i].payers.length + ' <b>ARPPU:</b> $' + Math.round(toDollars(groups[i].money) / groups[i].payers.length * 100) / 100 + '</span><br>';
        }
...
function getUniqueUsers() {
    ...
    var onReady = function(){
        if(uniqueXHR.readyState == 4 && uniqueXHR.status == 200 && uniqueiOSXHR.readyState == 4 && uniqueiOSXHR.status == 200 && uniqueAndroidXHR.readyState == 4 && uniqueAndroidXHR.status == 200) {
            ...
            var refund = (responseFb.Refund || 0) + (responseiOS.Refund || 0) + (responseAndroid.Refund || 0);
